...
import { newDataTypes } from '../src/config/types/new/index.js';
	...
	describe(`FV API Integration Tests for ${oldType} and ${newType}`, () => {
		it(`should handle ${'/' + oldType} route and create ${newType}`, async () => {
			try {
				...
				const { data, error } = await supabase
					...
					.eq(
						'fv_' + newDataTypeConfig.idField,
						response.body[0]['fv_' + newDataTypeConfig.idField],
					);
		...
		it(`should post and validate translated data for ${oldType} as ${newType}`, async () => {
			...
			const { data, error } = await supabase
				...
				.eq(
					newDataTypeConfig.idField,
					response.body[0][newDataTypeConfig.idField],
				);
