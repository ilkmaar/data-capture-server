...
export const translatePlayer = async (player, lookups = null) => {
    const fields = [
        { key: 'player_name', value: (p) => p.player_name }
    ];
    const references = [
        { key: 'world_id', extractor: (p) => p.world_id }
    ];
    return {
        ...createIdMapping('player', player.player_id),
        ...await translateBase(player, 'players', fields, references, lookups)
    };
};
...
export const translateCreature = async (creature, lookups = null) => {
    ...
    return {
        ...
        ...await translateBase(creature, 'creatures', fields, references, lookups)
    };
};
...
export const translateResource = async (resource, lookups = null) => {
    ...
    return {
        ...
        ...await translateBase(resource, 'items', fields, references, lookups)
    };
};
...
export const translateDataTable = async (table, lookups = null) => {
    const fields = [
        { key: 'data_table_name', value: (t) => t.table_name }
    ];
    const references = [
        { key: 'world_id', extractor: (t) => t.world_id }
    ];
    return {
        ...createIdMapping('data_table', table.table_id),
        ...await translateBase(table, 'player-manipulate-data-events', fields, references, lookups)
    };
};