	...
	const references = [
		{
			...
			extractor: (l) => l.location_creature_activity_type,
		},
...
export const translatePlot = async (plot, lookups = null) => {
	const fields = [];
	const references = [{ key: 'area_id', extractor: (p) => p.location_id }];
	return {
...
export const translatePatch = async (patch, lookups = null) => {
	const fields = [];
	const references = [{ key: 'plot_id', extractor: (p) => p.plot_id }];
	return {
...
export const translateResourceCategory = (resourceCategory) => ({
	...createIdMapping(
		'resource_category',
		resourceCategory.resource_category_id,
	),
...
export const translateResourceVariety = async (
	resourceVariety,
	lookups = null,
) => {
	const fields = [
	...
	return {
		...createIdMapping(
			'resource_variety',
			resourceVariety.resource_type_id,
		),
		...(await translateBase(
			resourceVariety,
			'resource-types',
...
export const translateResourceType = async (resourceType, lookups = null) => {
	...
	const references = [
		{
			key: 'resource_category_id',
			extractor: (rt) => rt.resource_category_id,
		},
...
export const translateItemType = async (itemType, lookups = null) => {
	const fields = [{ key: 'item_type_name', value: (it) => it.item_def_name }];
	const references = [
		{ key: 'item_category_id', extractor: (it) => it.item_category_id },
		{
			key: 'item_variety_id',
			extractor: (it) => lookupItemVarietyId(it.item_def_name),
		},
	];
	return {
